...
func TestMemoryInteractionStore_AppendAssignsSeq(t *testing.T) {
	...
	a, err := m.AppendInteraction("conv", "alice", renderable("i1"))
	if err != nil {
		t.Fatalf("append: %v", err)
	}
	b, err := m.AppendInteraction("conv", "alice", renderable("i2"))
	if err != nil {
		t.Fatalf("append: %v", err)
	}
	if a.Seq != 1 || b.Seq != 2 {
	...
	c, err := m.AppendInteraction("other", "bob", renderable("i3"))
	if err != nil {
		t.Fatalf("append: %v", err)
	}
	if c.Seq != 1 {
...
func TestMemoryInteractionStore_AppendIdempotentByID(t *testing.T) {
	m := NewMemoryInteractionStore()
	if _, err := m.AppendInteraction("conv", "alice", renderable("i1")); err != nil {
		t.Fatalf("append: %v", err)
	}
	// Answer it, then re-append the same id (e.g. an agent re-emit).
	if _, _, err := m.RecordInteractionResponse("conv", "i1", &pb.RenderableResponse{
		Id:     "i1",
		Action: pb.RenderableAction_RENDERABLE_ACTION_CANCEL,
	}); err != nil {
		t.Fatalf("record: %v", err)
	}
	again, err := m.AppendInteraction("conv", "alice", renderable("i1"))
	if err != nil {
...
func TestMemoryInteractionStore_GetFoundAndMissing(t *testing.T) {
	...
	got, found, err := m.GetInteraction("conv", "i1")
	if err != nil || !found {
	...
	if _, found, _ := m.GetInteraction("conv", "missing"); found {
		t.Errorf("missing interaction reported as found")
	}
	if _, found, _ := m.GetInteraction("nope", "i1"); found {
...
func TestMemoryInteractionStore_RecordIsIdempotent(t *testing.T) {
	...
	got, recorded, err := m.RecordInteractionResponse("conv", "i1", resp)
	if err != nil || !recorded {
	...
	second, recorded, err := m.RecordInteractionResponse("conv", "i1", &pb.RenderableResponse{
		Id:     "i1",
		Action: pb.RenderableAction_RENDERABLE_ACTION_CANCEL,
	})
	if err != nil {
...
func TestMemoryInteractionStore_RecordMissing(t *testing.T) {
	m := NewMemoryInteractionStore()
	_, _, err := m.RecordInteractionResponse("conv", "nope", &pb.RenderableResponse{Id: "nope"})
...
func TestMemoryInteractionStore_ListAndPending(t *testing.T) {
	...
	for _, id := range []string{"i1", "i2", "i3"} {
		if _, err := m.AppendInteraction("conv", "alice", renderable(id)); err != nil {
			t.Fatalf("append %s: %v", id, err)
		}
	}
	// Answer the middle one.
	if _, _, err := m.RecordInteractionResponse("conv", "i2", &pb.RenderableResponse{
		Id:     "i2",
		Action: pb.RenderableAction_RENDERABLE_ACTION_CANCEL,
	}); err != nil {
		t.Fatalf("record: %v", err)
	...
	pending, _ := m.PendingInteractions("conv")
	if len(pending) != 2 {
