...
Adds feedback logging for Slack thumbs up/down reactions and free-form comments
...
- a new `internal/feedbacklog` path that forwards trace-correlated feedback to
  Astro Server

The feedback log client is vendor-neutral from the messaging image's
perspective: it translates platform feedback into `thumbs_up`, `thumbs_down`, or
`comment` and skips events without trace context.

# Migration

None. Existing feedback callbacks continue to work. Feedback logging only occurs
